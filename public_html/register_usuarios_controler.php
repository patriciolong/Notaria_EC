...
if (!empty($_POST["btn_registro_user"])) {
    if (empty($_POST["usuario"]) || empty($_POST["nombre"]) || empty($_POST["apellido"]) || empty($_POST["pass"])|| empty($_POST["rol"])|| empty($_POST["estado"])) {
        ...
        $sql = $conexion->query("INSERT INTO usuario (u_nombre,u_apellido,u_usuario,u_contrasena,u_rol,u_estado)
        VALUES ('$nombre','$apellido','$usuario','$pass','$rol','$estado')");
